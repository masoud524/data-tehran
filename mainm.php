...
class tjgu{
  ...
  public function fetchDataAndSave() {
    ...
    $dirjs='json/'.$this->type.'/m';
    $direx='excel/'.$this->type.'/m';
    if (!is_dir($dirjs)) {
    ...
    file_put_contents($dirjs.'/'.$this->type.date("Y.m.d").'.json',$data);
    $xlsx->saveAs($direx.'/'.$this->type.date("Y.m.d").'.xlsx');
  }
}
